...
export class BitcoinCLI {
  ...
  private buildBaseCommand(): string {
    ...
    return `bitcoin-cli ${networkFlag} -rpcconnect=${this.config.rpchost} -rpcuser=${this.config.rpcuser} -rpcpassword=${this.config.rpcpassword} -rpcport=${this.config.rpcport} -rpcwallet=hamza-wallet`;
  }
